...
# Resource group (uncomment if you want creation here)
...
# --- Build Python deps into .python_packages for offline/fast deploys ---
pushd "$THIS_DIR/../function_app" >/dev/null
  ...
  # Build and cache wheels (reproducible installs / no network during install)
  if [[ -f requirements.txt ]]; then
    "$PYTHON_BIN" -m pip wheel -r requirements.txt -w wheelhouse

    # Vendor dependencies into Functions-friendly path
...
# --- Create deployment zip (rooted at function_app/) ---
...
az eventgrid event-subscription create \
  --name hns-quarantine-egsub \
  --source-resource-id "$STORAGE_ID" \
  --endpoint-type storagequeue \
  --endpoint "$QUEUE_ID" \
  --delivery-identity systemassigned \
  --included-event-types Microsoft.Storage.BlobCreated \
  --advanced-filter subjectEndsWith .exe .com .bat .cmd .scr .msi .msp .ps1 .ps2 .vbs .vbe .js .jse .wsf .wsh .hta .jar .dll .reg .cpl .lnk
EOF
